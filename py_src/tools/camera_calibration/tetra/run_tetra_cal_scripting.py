...
db_name = 'test_db'
path_to_images = sys.argv[1] #sys.argv[1]# r'C:\Users\Ethan\Desktop\COTS-Star-Tracker-Amalthea\external\test'#sys.argv[1]
# print(f"Echo: {sys.argv[1]}", file=sys.stdout)
# sys.exit()
# r'C:\Users\Ethan\Des ktop\COTS-Star-Tracker-Amalthea\external\test'
image_file_extension = sys.argv[2] #'.tiff'
darkframe_filename = 'autogen_darkframe.jpg'
estimated_full_angle_fov = 27.7 # FoV (x,y): 32.34193580608777, 24.397826957559573   32 removed
verbose = True
...
for impath in image_files:
        ...
        num_images+=1
        print("Attempting to solve image " + str(num_images)+" of "+str(len(image_files))+": " + str(impath), file=sys.stdout)
        ...
        if solved['Matches'] is not None:
            ...
            ###################################################################################
            print("Success:{} ".format(str(impath)), file=sys.stdout)
            ###################################################################################
...
###################################################################################
print("Solve Rate:{}".format(float(len(solution_list))/float(len(image_files))), file=sys.stdout)
    ###################################################################################
...
if len(solution_list) > 0:
    ...
    ###################################################################################
    print("RMS:" + str(RMS_reproj_err_pix), file=sys.stdout)
    ###################################################################################
    ...
    #save if it's good
    usr_in = print('\n\n\nIf you are satisfied with the results, provide a file name.  Otherwise, enter "no". [no/file_name]: ', file=sys.stdout)

    stdin_bytes = sys.stdin.buffer.read()  # Read raw bytes

    try:
        # Try decoding as UTF-8 (for QProcess.write)
        usr_in = stdin_bytes.decode("utf-8").strip().lower()
    except UnicodeDecodeError:
        # If decoding fails, assume it's already a string (unlikely in this case)
        # Alternatively, read again in text mode (but this may not work if stdin is already consumed)
        usr_in = sys.stdin.readline().strip().lower()
    """
    stdin = sys.stdin.read()
    if stdin is type bytes:
        usr_in = bytes(stdin).decode("utf8")
    else:
